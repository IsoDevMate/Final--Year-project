...
try {
    ...
    if (!process.env.FIREBASE_CONFIG) {
        ...
        throw new errors_utils_1.AppError('Firebase configuration is missing', 500);
    }
    // Parse the base64-encoded config
    firebaseConfig = JSON.parse(Buffer.from(process.env.FIREBASE_CONFIG, 'base64').toString('utf-8'));
    if (!firebaseConfig) {
        throw new errors_utils_1.AppError('Firebase configuration is invalid or empty', 500);
    }
    ...
    admin.initializeApp({
        ...
        storageBucket: config_1.default.storagebucketnameis,
    });
...
class StorageService {
    constructor() {
        this.bucketName = config_1.default.storagebucketnameis;
    ...
    checkInitialization() {
        if (!this.isInitialized || !this.bucket) {
            throw new errors_utils_1.AppError('Storage service is not properly initialized', 500);
        }
    }
    uploadFile(file, fileName, userId, fileType) {
        return __awaiter(this, void 0, void 0, function* () {
            this.checkInitialization();
            try {
                if (!file || !Buffer.isBuffer(file)) {
                    throw new errors_utils_1.AppError('Invalid file data provided', 400);
                }
                if (!fileName) {
                    throw new errors_utils_1.AppError('File name is required', 400);
                }
                const fileExtension = fileName.split('.').pop() || '';
                ...
                if (error instanceof Error) {
                    throw new errors_utils_1.AppError(`File upload failed: ${error.message}`, 500);
                }
                throw new errors_utils_1.AppError('File upload failed', 500);
            }
        });
    }
    deleteFile(storageRef) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (!storageRef) {
                    throw new errors_utils_1.AppError('Storage reference is required', 400);
                }
                const fileRef = this.bucket.file(storageRef);
                ...
                if (error instanceof Error) {
                    throw new errors_utils_1.AppError(`File deletion failed: ${error.message}`, 500);
                }
                throw new errors_utils_1.AppError('File deletion failed', 500);
            }
        });
    }
    getContentType(fileType, extension) {
        const types = {
            image: {
                jpg: 'image/jpeg',
                jpeg: 'image/jpeg',
