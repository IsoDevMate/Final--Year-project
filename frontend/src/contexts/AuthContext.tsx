...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  try {
    if (isTokenValid()) {
      ...
      const response = await axios.get(`https://final-year-project-56d5.onrender.com/api/v1/auth/me`, {
 ...
 const login = async (email: string, password: string) => {
  try {
    const response = await axios.post(`https://final-year-project-56d5.onrender.com/api/v1/auth/login`, {
      email,
      password
    });
  ...
  const loginWithLinkedIn = async () => {
    try {
      ...
      window.location.href = `https://final-year-project-56d5.onrender.com/api/v1/auth/linkedin`;
    } catch (error) {
  ...
  const logout = async () => {
    try {
      await axios.post(`https://final-year-project-56d5.onrender.com/api/v1/auth/logout`);
      localStorage.removeItem('accessToken');
      localStorage.removeItem('refreshToken');
      localStorage.removeItem('user');
      setUser(null);
  ...
  const register = async (email: string, password: string, name: string) => {
    try {
      const response = await axios.post(`https://final-year-project-56d5.onrender.com/api/v1/auth/register`, {
        email,
        password,
        name
      });
