  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/v1/events?${params.toString()}`);
...
const handleDeleteEvent = async (eventId: string) => {
  if (window.confirm('Are you sure you want to delete this event?')) {
    try {
      ...
      const response = await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/v1/events/${eventId}`, {
        headers: {
  ...
  const registerFreeEvent = async (eventId: string) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/v1/events/register/${eventId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`
