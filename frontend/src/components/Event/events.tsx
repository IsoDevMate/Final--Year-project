...
// import { Calendar, MapPin, Users, Clock, Filter, Search, Plus, Trash2, Edit } from 'lucide-react';
...
// const eventTypes = ['conference', 'workshop', 'meetup', 'webinar', 'training', 'other'];
...
//         const response = await axios.get(`http://localhost:3000/api/v1/events?${params.toString()}`);
...
//       try {
//         const response = await axios.delete(`http://localhost:3000/api/v1/events/${eventId}`);
...
//   try {
//     // Get user from the auth context (already available from useAuth())
//     if (!user) {
...
//     // Get user ID from the auth context user
//     const currentUserId = user.id; // Or whatever property holds the ID in your user object
...
//     // Get token (either from auth context or localStorage)
...
//       `http://localhost:3000/api/v1/events/register/${eventId}`,
//       {},
//       {
//         headers: {
...
//     } else {
//       alert(response.data.message || 'Failed to register for the event');
//     }
//   } catch (err: any) {
...
//                           className="text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1 rounded-md mr-4"
//                           disabled={Array.isArray(event.attendees) && event.attendees.includes(userId)}
//                         >
//                           {Array.isArray(event.attendees) && event.attendees.includes(userId) ? 'Registered' : 'Register'}
//                         </button>
//                         <button
...
const EventsPage = () => {
  ...
  // const NavigateToEventSuccess = (eventId: string) => {
  //   navigate(`/frontenddashboard/events/${eventId}/success`);
  // }
