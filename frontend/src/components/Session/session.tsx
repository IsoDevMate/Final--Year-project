...
const SessionsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch event details
        const eventResponse = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/v1/events/${eventId}`);
        ...
        const sessionsResponse = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/v1/sessions/event/${eventId}`);
  ...
  const handleDeleteSession = async (sessionId: string) => {
    if (window.confirm('Are you sure you want to delete this session?')) {
      try {
        const response = await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/v1/sessions/${sessionId}`);
  ...
  const handleToggleLiveStream = async (sessionId: string, isCurrentlyLiveStreamed: boolean, streamUrl?: string) => {
    try {
      ...
      const response = await axios.patch(`${import.meta.env.VITE_API_BASE_URL}/api/v1/sessions/${sessionId}/livestream`, updateData);
  ...
  const handleRegisterForSession = async (sessionId: string) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/v1/sessions/${sessionId}/register`);
