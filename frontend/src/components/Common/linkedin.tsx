...
const LinkedInShareButton: React.FC<LinkedInShareButtonProps> = ({ noteId }) => {
  ...
  useEffect(() => {
    const checkLinkedInStatus = async () => {
      try {
        const response = await axios.get('https://final-year-project-77pa.onrender.com/api/v1/linkedin/status');
  ...
  const handleShare = async () => {
    ...
    try {
      const response = await axios.post(`https://final-year-project-77pa.onrender.com/api/v1/linkedin/share/note/${noteId}`);
